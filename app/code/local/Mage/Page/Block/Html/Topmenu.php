    ...
    protected function _getHtml(Varien_Data_Tree_Node $menuTree, $childrenWrapClass)
    {
        ...
        foreach ($children as $child) {
            ...
            if ($childLevel == 0 && $outermostClass) {
                ...
                $url = Mage::getBaseUrl('media').'category/'.$child->getData('thumbnail'); // Edit
                $img = '<img src"'.$urls.'" width="50px" height="50px"/>';                  // Edit
            }

            $html .= '<li id="navmenu-img" ' . $this->_getRenderedMenuItemAttributes($child) . '>';
            $html .= '<a href="" ' . $outermostClassCode . '>' .$img. '<span>'. $this->escapeHtml($child->getName()) . '</span></a>';             //.$img. Edit
