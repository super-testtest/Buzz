...
class CategoryImporter extends Magmi_ItemProcessor
{
    ...
    protected $_category_header = '_category';
    ...
    public function getStoreRootPaths(&$item)
    {
        ...
        if (preg_match_all("|\[(.*?)\]|", $item[$this->_category_header], $matches)) {
            $cm1 = count($matches[1]);
            ...
            for ($i = 0; $i < $cm1; $i++) {
                ...
                foreach ($sids as $sid) {
                    ...
                    if ($cmatch) {
                        ...
                        $item[$this->_category_header] = str_replace($matches[0][$i], $k, $item[$this->_category_header]);
                        break;
                    }
                }
            }
    ...
    {
        if (isset($item[$this->_category_header])) {
            // handle escaping
            $icats = $this->processEscaping($item[$this->_category_header]);
            ...
            if ($root != "") {
                ...
                $item[$this->_category_header] = implode(";;", $catlist);
            }
