...
function amshopby_ajax_init(){
    ...
    if (isRwd) {
        ...
        var isUltimo = typeof enquire == 'object' && typeof ProductMediaManager == 'object' ;
        if (isUltimo) amshopby_ajax_ultimo_fix();
    }
}
...
function amshopby_ajax_ultimo_fix() {
    if ($j('.block-layered-nav').length && $j('.category-products').length) {
        var query = 'screen and (max-width: 770px)';

        var register = function() {
                enquire.register(query, {
                    match: function () {
                        setTimeout(function() {
                            $j('.block-layered-nav').insertAfter($j('.breadcrumbs'));
                            $j('.block-layered-nav .toggle-content').toggleSingle();
                        }, 600);
                    }
                });
        }
